...
type FileEventHandler interface {
	OnFileChanged(ctx context.Context, filepath string)
	OnFileDeleted(ctx context.Context, filepath string)
}
...
func NewFileWatcher(handler FileEventHandler) (*FileWatcher, error) {
...
func (w *FileWatcher) WatchSingleFile(ctx context.Context, filePath string, debounceMs int) error {
	...
	logger.WithValues("filePath", filePath).Info("started watching file")
...
func (w *FileWatcher) WatchOptionalFile(ctx context.Context, filePath string, debounceMs int) error {
	logger := log.FromContext(ctx)
	if filePath == "" {
		logger.Info("no file path provided, waiting for graceful termination")
		<-ctx.Done()
...
func (w *FileWatcher) WatchDirectory(ctx context.Context, dirPath string) error {
	...
	logger.WithValues("dirPath", dirPath).Info("started watching directory")
...
func (w *FileWatcher) watchWithDebounce(ctx context.Context, targetFile string, debounceDelay time.Duration) error {
	...
	for {
		select {
		case <-ctx.Done():
			logger.Info("stopping file watcher gracefully")
			return nil // Graceful termination is not an error
		case event, ok := <-w.watcher.Events:
			...
			if w.isTargetFileEvent(event, targetFile) {
				logger.V(4).WithValues("event", event.String()).Info("file changed")
				...
				debounceTimer = time.AfterFunc(debounceDelay, func() {
					w.handler.OnFileChanged(ctx, targetFile)
				})
			}
...
func (w *FileWatcher) handleEvent(ctx context.Context, event fsnotify.Event) {
	...
	case fsnotify.Create, fsnotify.Write:
		...
		if err == nil && !info.IsDir() {
			w.handler.OnFileChanged(ctx, filePath)
		}
		if err == nil && info.IsDir() {
			...
			err = filepath.WalkDir(filePath, func(path string, d fs.DirEntry, err error) error {
				...
				w.handler.OnFileChanged(ctx, path)
	...
	case fsnotify.Rename, fsnotify.Remove:
		w.handler.OnFileDeleted(ctx, filePath)
	default:
